...
function AuditTable({ rows, compact = false }: { rows: any[]; compact?: boolean }) {
  ...
  return <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead><tr className="border-b border-[#e7e4db] font-mono-ui text-[9px] uppercase tracking-[.14em] text-[#8a9a9b]"><th className="px-5 py-3 font-bold">Payload preview</th><th className="px-3 py-3 font-bold">Severity</th><th className="px-3 py-3 font-bold">Privacy</th><th className="px-3 py-3 font-bold">Signals</th><th className="px-3 py-3 font-bold">Time</th><th className="px-5 py-3 text-right font-bold">Inspected</th></tr></thead><tbody>{safeRows.map((row, i) => <tr key={row.id ?? i} data-testid={`row-audit-${row.id ?? i}`} className="border-b border-[#ebe8df] last:border-0 transition-colors hover:bg-[#f5f3eb]"><td className="max-w-[300px] px-5 py-4"><div className="flex items-center gap-3"><div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-[#e6f0eb] text-[#438d76]"><Terminal size={13} /></div><span className="audit-payload-preview truncate font-mono-ui text-[11px] text-[#385762]">{row.preview ?? 'Inspection'}</span></div></td><td className="px-3 py-4"><SeverityPill severity={row.severity} small /></td><td className="px-3 py-4"><span className={`font-mono-ui text-xs font-bold ${row.privacyScore < 60 ? 'text-[#a55047]' : row.privacyScore < 80 ? 'text-[#986822]' : 'text-[#287459]'}`}>{row.privacyScore ?? 100}</span><span className="audit-table-muted text-[10px] text-[#849394]">/100</span></td><td className="audit-table-muted px-3 py-4 font-mono-ui text-xs text-[#506c73]">{row.threatCount ?? 0}</td><td className="audit-table-muted px-3 py-4 font-mono-ui text-xs text-[#506c73]">{row.processingTimeMs ?? 0}ms</td><td className="audit-table-muted px-5 py-4 text-right text-[11px] text-[#849394]">{formatDate(row.createdAt ?? '')}</td></tr>)}</tbody></table></div>;
}
...
function Inspector() {
  ...
  const resetPayload = () => { setPayload(''); setResult(null); setPdfName(''); setPdfStatus(''); };
...
function ResultPanel({ result, payload, recipientEmail }: {
  result: any;
  payload: string;
    ...
    <section className="animate-rise delay-1 overflow-hidden rounded-2xl border border-[#e1ded5] bg-[#faf9f3] paper-shadow">
      ...
      <div className="border-t border-[#e7e4db] bg-[#f7f5ed] p-5 md:p-7">
        <div className="mb-3 flex flex-wrap gap-2">
          {quickPrompts.map((prompt) => <button key={prompt} type="button" onClick={() => setDraft(prompt)} className="assistant-quick-prompt rounded-full border border-[#d8e2d8] bg-[#eef5ed] px-3 py-1.5 text-left text-[10px] font-semibold text-[#477269] transition hover:border-[#65af93] hover:bg-[#e4f1e6]">{prompt}</button>)}
        </div>
        <form onSubmit={submit} className="flex items-end gap-3">
          <label htmlFor="assistant-message" className="sr-only">Message the security assistant</label>
          <textarea id="assistant-message" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} rows={2} maxLength={4000} placeholder="Ask a privacy or security question…" className="assistant-composer min-h-[54px] flex-1 resize-none rounded-xl border border-[#d7ddd5] bg-[#fcfcf6] px-4 py-3 text-sm text-[#315462] outline-none transition-colors placeholder:text-[#a4b0aa] focus:border-[#65af93] focus:ring-2 focus:ring-[#bde5d3]" />
          <button type="submit" disabled={!draft.trim() || sendMessage.isPending} data-testid="button-send-assistant" className="assistant-send flex h-[54px] items-center gap-2 rounded-xl bg-[#183f4f] px-4 text-xs font-bold text-[#f6f4eb] shadow-[0_4px_0_#0d2b38] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-45">{sendMessage.isPending ? <RefreshCw size={14} className="animate-spin" /> : <Send size={14} />}<span className="hidden sm:inline">Send</span></button>
        </form>
